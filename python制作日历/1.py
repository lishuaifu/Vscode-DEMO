...
def main():
	# 读取 excel 文件
	data = xlrd.open_workbook("D:\python\python制作日历\classInfo.xlsx")
	table = data.sheets()[0]
	...
	while i < numOfRow :
		...
		s = int((table.cell(i, _colOfStartWeek).value))
		e = int((table.cell(i, _colOfEndWeek).value))
		w = int((table.cell(i, _colOfWeekday).value))
		if w == 7:
			s -= 1
			e -= 1
		startWeekList.append(str(s))
		endWeekList.append(str(e))
		weekdayList.append(str(w))
	...
	# print classInfoStr
	with open("D:\python\python制作日历\conf_classInfo.json",'w') as f:
